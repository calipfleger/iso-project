...
df = pd.read_csv(datadir + 'iLIRA_mean_afedit.csv', sep = ',')
sst = np.array(df["TS"])
# Load SST anomalies [K] (NOTE: THIS SHOULD BE A 1-D VECTOR OF DATA!)
# yearly
df_sst_a = pd.read_csv(datadir + 'ssta_LIRAiM_binned.csv', sep = ',')
...
ssta_m.to_csv(datadir+"SSTA_LIRAiM_M.csv", index = False, header = False)

# Load SSS anomalies [psu] (NOTE: THIS SHOULD BE A 1-D VECTOR OF DATA!)
# yearly
df_sss_a = pd.read_csv(datadir + 'ssta_LIRAiM_binned.csv', sep = ',')
...
sssa_m.to_csv(datadir+"SSSA_LIRAiM_M.csv", index = False, header = False)
...
#====================================================================
# Save coral timeseries fields as LIRAy arrays in current directory.
outdir = './results/'
print('Saving time series...')
np.save(outdir + "LIRAiM_coral_d18O.npy", coral)
np.save(outdir + "LIRAiM_coral_age_perturbed.npy", Xp)
coral = pd.DataFrame(coral)
coral.to_csv(outdir+"LIRAiM_pseudocoral.csv", index = False, header = False)
