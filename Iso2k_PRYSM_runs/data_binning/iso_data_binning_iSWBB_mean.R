...
ssta_m = read.csv("iSWBB_mean_afedit.csv", head = T)[3]
start = 1851
end = 1981
...
write.csv(binned_ann_recs, "ssta_SWBBiM_binned.csv")

# Bin monthly sss
sssa_m = read.csv("iSWBB_mean_afedit.csv", head = T)[2]
start = 1851
end = 1981
time = seq(from = start, to = end, by = 1/12)
...
write.csv(binned_ann_recs, "sssa_SWBBiM_binned.csv")
