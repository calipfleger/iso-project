...
sst = read.csv("ssta_SWBBiM_binned.csv", head = T)
sss = read.csv("sssa_SWBBiM_binned.csv", head = T)
...
coral = read.csv("SWBBiM_pseudocoral.csv", head = F)
...
plot(sst$x, sst_c, type = "l", frame.plot = F, xaxt = "n", xlab = "", lwd = 2,
     ylab = "iCESM SST (C)", xpd = NA, main = swbb, 
