...
sst = read.csv("ssta_NUMPiM_binned.csv", head = T)
sss = read.csv("sssa_NUMPiM_binned.csv", head = T)
...
coral = read.csv("NUMPiM_pseudocoral.csv", head = F)
...
plot(sst$x, sst_c, type = "l", frame.plot = F, xaxt = "n", xlab = "", lwd = 2,
     ylab = "iCESM SST (C)", xpd = NA, main = nump, 
