...
df = pd.read_csv(datadir + 'iNUMP_mean_afedit.csv', sep = ',')
sst = np.array(df["TS"])
# Load SST anomalies [K] (NOTE: THIS SHOULD BE A 1-D VECTOR OF DATA!)
# yearly
df_sst_a = pd.read_csv(datadir + 'ssta_NUMPiM_binned.csv', sep = ',')
...
ssta_m.to_csv(datadir+"SSTA_NUMPiM_M.csv", index = False, header = False)

# Load SSS anomalies [psu] (NOTE: THIS SHOULD BE A 1-D VECTOR OF DATA!)
# yearly
df_sss_a = pd.read_csv(datadir + 'ssta_NUMPiM_binned.csv', sep = ',')
...
sssa_m.to_csv(datadir+"SSSA_NUMPiM_M.csv", index = False, header = False)
...
np.save(outdir + "NUMPiM_coral_d18O.npy", coral)
np.save(outdir + "NUMPiM_coral_age_perturbed.npy", Xp)
coral = pd.DataFrame(coral)
coral.to_csv(outdir+"NUMPiM_pseudocoral.csv", index = False, header = False)
