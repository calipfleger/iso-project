...
df = pd.read_csv(datadir + 'iSWBB_mean_afedit.csv', sep = ',')
sst = np.array(df["TS"])
# Load SST anomalies [K] (NOTE: THIS SHOULD BE A 1-D VECTOR OF DATA!)
# yearly
df_sst_a = pd.read_csv(datadir + 'ssta_SWBBiM_binned.csv', sep = ',')
...
ssta_m.to_csv(datadir+"SSTA_SWBBiM_M.csv", index = False, header = False)

# Load SSS anomalies [psu] (NOTE: THIS SHOULD BE A 1-D VECTOR OF DATA!)
# yearly
df_sss_a = pd.read_csv(datadir + 'ssta_SWBBiM_binned.csv', sep = ',')
...
sssa_m.to_csv(datadir+"SSSA_SWBBiM_M.csv", index = False, header = False)
...
#====================================================================
# Save coral timeseries fields as SWBBy arrays in current directory.
outdir = 'C:/Users/andre/Documents/GitHub/iso-project/Iso2k_PRYSM_runs/results/'
print('Saving time series...')
np.save(outdir + "SWBBiM_coral_d18O.npy", coral)
np.save(outdir + "SWBBiM_coral_age_perturbed.npy", Xp)
coral = pd.DataFrame(coral)
coral.to_csv(outdir+"SWBBiM_pseudocoral.csv", index = False, header = False)
